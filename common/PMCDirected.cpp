...
namespace multistateTurnip
{
	...
	{
		const ContextDirected::internalGraph& directedGraph = args.context.getGraph();
		...
		edmondsKarpMaxFlow<ContextDirected::internalGraph, double>(&capacityVector.front(), &flowVector.front(), &residualVector.front(), directedGraph, source, sink, args.threshold, scratch, minimumPossibleFlow);
		...
		for(int i = 0; i < args.n; i++)
		{
			...
			while(insufficientFlow && repairTimeIterator != repairTimes.end())
			{
				...
				double newCapacity = std::max(capacityVector[edge], (cumulativeData.rbegin() + level)->first);
				...
				edmondsKarpMaxFlow<ContextDirected::internalGraph, double>(&capacityVector.front(), &flowVector.front(), &residualVector.front(), directedGraph, source, sink, args.threshold, scratch, currentFlow);
