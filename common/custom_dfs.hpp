...
//=======================================================================
//
#ifndef CUSTOM_BOOST_GRAPH_UNDIRECTED_DFS_HPP
#define CUSTOM_BOOST_GRAPH_UNDIRECTED_DFS_HPP
...
// comparison.
	...
	if(stack.size() > 0 && stack.back().second.first)
	{
		Edge tmp = stack.back().second.first.get();
		call_finish_edge(vis, tmp, g);
	}
      }
    }
       ...
       EdgeColorMap edge_color)
    {
      ...
      for (boost::tie(ei, ei_end) = out_edges(u, g); ei != ei_end; ++ei) {
        ...
        if (v_color == Color::white()) {     vis.tree_edge(*ei, g);
          custom_undir_dfv_impl(g, v, vis, vertex_color, edge_color);
  ...
  template <typename Graph, typename DFSVisitor, 
            ...
            typename Vertex>
  void
  custom_undirected_dfs(const Graph& g, DFSVisitor vis, 
  ...
  custom_undirected_dfs(const Graph& g, DFSVisitor vis, 
                 VertexColorMap vertex_color, EdgeColorMap edge_color)
  {
    custom_undirected_dfs(g, vis, vertex_color, edge_color, *vertices(g).first);
  }

  namespace detail {
    template <typename VertexColorMap>
    struct custom_udfs_dispatch {
      ...
      apply(const Graph& g, DFSVisitor vis, Vertex start_vertex,
            ...
            VertexColorMap vertex_color)
      {
        custom_undirected_dfs(g, vis, vertex_color, edge_color, start_vertex);
      }
    };

    template <>
    struct custom_udfs_dispatch<param_not_found> {
  ...
  void
  custom_undirected_dfs(const Graph& g, 
                 const bgl_named_params<P, T, R>& params)
  {
    typedef typename get_param_type< vertex_color_t, bgl_named_params<P, T, R> >::type C;
    detail::custom_udfs_dispatch<C>::apply
  ...
  template <typename IncidenceGraph, typename DFSVisitor, 
    typename VertexColorMap, typename EdgeColorMap>
  void custom_undirected_depth_first_visit
    (const IncidenceGraph& g,
     typename graph_traits<IncidenceGraph>::vertex_descriptor u, 
     DFSVisitor vis, VertexColorMap vertex_color, EdgeColorMap edge_color)
  {
    detail::custom_undir_dfv_impl(g, u, vis, vertex_color, edge_color);
  }
