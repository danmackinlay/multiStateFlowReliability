...
for(currentGraph in graphs)
{
	...
	workNormalizedVarianceData <- melt(cbind(currentGraphScenarios, workNormalizedVariance = workNormalizedVariance[currentGraphScenariosIndices]), measure.vars = "workNormalizedVariance")
	...
	for(nCapacityLevels in nCapacityLevels)
	{
		...
		for(currentDemand in demands)
		{
			...
			currentWorkNormalizedVarianceData <- subset(workNormalizedVarianceData, nCapacities == nCapacityLevels & demand == currentDemand & method != "crudeMC")
			workNormalizedVarianceDataPlot <- ggplot(data=currentWorkNormalizedVarianceData, aes(x = epsilon, y = log10(value), group = method, colour = method)) + guides(col=guide_legend(ncol = 2, title=NULL)) + xlab("epsilon") + ylab("Work normalized variance") + geom_line() + scale_colour_manual(values = colours) + scale_x_discrete(name = "epsilon") + theme(legend.position = c(0.77, 0.9)) + scale_y_continuous(labels = powerFormatter) + ggtitle(paste0("Work Normalized Variance, Demand = ", currentDemand, ", Capacity levels = ", nCapacityLevels))
			texFile <- paste0("./workNormalizedVariance-", nCapacityLevels, "-", currentDemand, ".tex")
			tikz(texFile, width = 7, height = 7, standAlone = TRUE, packages = c("\\usepackage{amssymb}", "\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}"))
				print(workNormalizedVarianceDataPlot)
			dev.off()
			system(paste0("pdflatex ", texFile))
			unlink(paste0("./workNormalizedVariance-", nCapacityLevels, "-", currentDemand, ".tex"))
			unlink(paste0("./workNormalizedVariance-", nCapacityLevels, "-", currentDemand, ".aux"))
			unlink(paste0("./workNormalizedVariance-", nCapacityLevels, "-", currentDemand, ".log"))
