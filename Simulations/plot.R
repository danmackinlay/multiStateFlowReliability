...
for(nCapacityLevels in nCapacityLevels)
{
	...
	for(currentDemand in demands)
	{
		...
		currentWorkNormalizedSingleSampleVarianceData <- subset(workNormalizedSingleSampleVarianceData, nCapacities == nCapacityLevels & demand == currentDemand & method != "crudeMC" & method != "pmc1")
		workNormalizedSingleSampleVariancePlot <- ggplot(data=currentWorkNormalizedSingleSampleVarianceData, aes(x = epsilon, y = log10(value), group = method, colour = method)) + guides(col=guide_legend(ncol = 2, title=NULL)) + xlab("epsilon") + ylab("Work Normalized Variance Per Sample") + geom_line() + scale_colour_manual(values = colours) + scale_x_discrete(name = "epsilon") + theme(legend.position = c(0.77, 0.9)) + scale_y_continuous(labels = powerFormatter) + ggtitle("Average estimates")
